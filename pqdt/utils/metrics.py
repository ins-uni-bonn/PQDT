...
class Metrics(object):
    ITEMS = [{
        'name': 'F-Score',
        'enabled': True,
        'eval_func': '_get_f_score',
        'is_greater_better': True,
        'init_value': 0
    }, {
        'name': 'CDL1',
        'enabled': True,
        'eval_func': '_get_chamfer_distancel1',
        ...
        'name': 'CDL2',
        'enabled': True,
        'eval_func': '_get_chamfer_distancel2',
    ...
    def get(cls, pred, gt, require_emd=False):
        ...
        for i, item in enumerate(_items):
            if not require_emd and 'emd' in item['eval_func']:
                ...
                eval_func = getattr(cls, item['eval_func'])
    ...
    @classmethod
    def _get_f_score(cls, pred, gt, th=0.005):
    ...
    @classmethod
    def _get_emd_distance(cls, pred, gt, eps=0.01, iterations=1000):
        emd_loss = cls.ITEMS[3]['eval_object']
        if pred.size(1) > gt.size(1):
           pred = fps_subsample(pred, gt.shape[1]) 
        elif pred.size(1) < gt.size(1):
            #gt = fps_subsample(gt, pred.shape[1])
            gt = fps_subsample(gt, 7680)
            pred = fps_subsample(pred, 7680)
