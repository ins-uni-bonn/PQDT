...
class GeometricStructureEmbedding(nn.Module):
    ...
    def forward(self, points, group_idx=None):
        r"""Geometric structure embedding.
        Args:
            points: torch.Tensor (B, N, 3), input point cloud
            group_idx: torch.Tensor (B, N), group index for grouping
        Returns:
            embeddings: torch.Tensor (B, N, k, c), geometric structure embeddings
        """
        d_indices, a_indices = self.get_embedding_indices(points)
        if group_idx is not None:
            d_indices = torch.gather(d_indices, 2, group_idx)
            a_indices = torch.gather(a_indices, 2, group_idx.unsqueeze(-1).expand(-1, -1, -1, a_indices.size(-1)))
